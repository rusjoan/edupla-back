...
server.exchange(oauth2orize.exchange.refreshToken(function(client, refreshToken, scope, done) {
		...
		RefreshToken.findOne({ token: refreshToken }, function(err, token) {
			...
			User.findOne({ id: token.userId }, function(err, user) {
				...
				// Remove Refresh and Access tokens and create new ones
				RefreshToken.destroy({ userId: user.id, clientId: code.clientId }, function (err) {
					if (err) return done(err);
					AccessToken.destroy({ userId: user.id, clientId: code.clientId }, function (err) {
						if (err) return done(err);
						RefreshToken.create({ userId: user.id, clientId: code.clientId }, function(err, refreshToken) {
							if (err) return done(err);
							AccessToken.create({ userId: user.id, clientId: code.clientId }, function(err, accessToken) {
								if (err) return done(err);
								return done(null, accessToken.token, refreshToken.token, { 'expires_in': sails.config.oauth.tokenLife });
							});
						});
